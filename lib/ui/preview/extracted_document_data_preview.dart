...
class ExtractedDocumentDataPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
      ...
      switch (document.type.name) {
        case MRZ.DOCUMENT_TYPE:
          ...
          addField('Birth Date', mrz.birthDate.value?.text,
              mrz.birthDate.value?.confidence);
          addField('Expiry Date', mrz.expiryDate?.value?.text,
              mrz.expiryDate?.value?.confidence);
          addField('Given Names', mrz.givenNames.value?.text,
              mrz.givenNames.value?.confidence);
          addField('TravelDocType', mrz.travelDocType?.value?.text,
              mrz.travelDocType?.value?.confidence);
          addField('Document Number', mrz.documentNumber?.value?.text,
              mrz.documentNumber?.value?.confidence);
          addField('Nationality', mrz.nationality.value?.text,
              mrz.nationality.value?.confidence);
          addField(
              'Pin', mrz.pinCode?.value?.text, mrz.pinCode?.value?.confidence);
          addField('Surname', mrz.surname.value?.text,
              mrz.surname.value?.confidence);
          break;
        case DeHealthInsuranceCardFront.DOCUMENT_TYPE:
          ...
          addField('CardAccessNumber', hic.cardAccessNumber?.value?.text,
              hic.cardAccessNumber?.value?.confidence);
          addField('IssuerName', hic.issuerName.value?.text,
              hic.issuerName.value?.confidence);
          addField('IssuerNumber', hic.issuerNumber.value?.text,
              hic.issuerNumber.value?.confidence);
          addField('PersonalNumber', hic.personalNumber.value?.text,
              hic.personalNumber.value?.confidence);
          break;
        ...
        case DeResidencePermitBack.DOCUMENT_TYPE:
          ...
          addField('Address', doc.address.value?.text,
              doc.address.value?.confidence);
          addField('Birth Date', doc.birthDate?.value?.text,
              doc.birthDate?.value?.confidence);
          addField('Eye Color', doc.eyeColor.value?.text,
              doc.eyeColor.value?.confidence);
          addField(
              'Gender', doc.gender?.value?.text, doc.gender?.value?.confidence);
          addField(
              'Height', doc.height.value?.text, doc.height.value?.confidence);
          addField('Issuing Authority', doc.issuingAuthority.value?.text,
              doc.issuingAuthority.value?.confidence);
          addField('Nationality', doc.nationality?.value?.text,
              doc.nationality?.value?.confidence);
          addField(
              'Raw MRZ', doc.rawMRZ.value?.text, doc.rawMRZ.value?.confidence);
          addField('Remarks', doc.remarks?.value?.text,
              doc.remarks?.value?.confidence);
          break;
        case DeIdCardBack.DOCUMENT_TYPE:
          ...
          addField('Issue Date', doc.issueDate.value?.text,
              doc.issueDate.value?.confidence);
          addField('Issuing Authority', doc.issuingAuthority.value?.text,
              doc.issuingAuthority.value?.confidence);
          addField('Pseudonym', doc.pseudonym?.value?.text,
              doc.pseudonym?.value?.confidence);
          addField(
              'Raw MRZ', doc.rawMRZ.value?.text, doc.rawMRZ.value?.confidence);
          break;
        case DePassport.DOCUMENT_TYPE:
          ...
          addField('Country Code', doc.countryCode.value?.text,
              doc.countryCode.value?.confidence);
          addField('Expiry Date', doc.expiryDate.value?.text,
              doc.expiryDate.value?.confidence);
          addField(
              'Gender', doc.gender.value?.text, doc.gender.value?.confidence);
          ...
          addField('Passport Type', doc.passportType.value?.text,
              doc.passportType.value?.confidence);
          addField('Surname', doc.surname.value?.text,
              doc.surname.value?.confidence);
          addField(
              'Raw MRZ', doc.rawMRZ.value?.text, doc.rawMRZ.value?.confidence);
        ...
        case EuropeanHealthInsuranceCard.DOCUMENT_TYPE:
          ...
          addField('Barcode', doc.barcode?.value?.text,
              doc.barcode?.value?.confidence);
          addField('BirthDate', doc.expiryDate.value?.text,
              doc.expiryDate.value?.confidence);
          addField('CardNumber', doc.cardNumber.value?.text,
              doc.cardNumber.value?.confidence);
          addField('CountryCode', doc.countryCode.value?.text,
              doc.countryCode.value?.confidence);
          addField('ExpiryDate', doc.expiryDate.value?.text,
              doc.expiryDate.value?.confidence);
          addField('GivenNames', doc.givenNames.value?.text,
              doc.givenNames.value?.confidence);
          addField('IssuerName', doc.issuerName.value?.text,
              doc.issuerName.value?.confidence);
          addField('IssuerNumber', doc.issuerNumber.value?.text,
              doc.issuerNumber.value?.confidence);
          addField('PersonalNumber', doc.personalNumber.value?.text,
              doc.personalNumber.value?.confidence);
          addField('Signature', doc.signature?.value?.text,
              doc.signature?.value?.confidence);
          addField('Surname', doc.surname.value?.text,
              doc.surname.value?.confidence);
          break;
        case EuropeanDriverLicenseFront.DOCUMENT_TYPE:
          ...
          addField('GivenNames', doc.givenNames.value?.text,
              doc.givenNames.value?.confidence);
          addField('LicenseCategories', doc.licenseCategories?.value?.text,
              doc.licenseCategories?.value?.confidence);
          addField('Photo', doc.photo.value?.text, doc.photo.value?.confidence);
          addField('SerialNumber', doc.serialNumber?.value?.text,
              doc.serialNumber?.value?.confidence);
          addField('Signature', doc.signature.value?.text,
              doc.signature.value?.confidence);
          addField('Surname', doc.surname.value?.text,
              doc.surname.value?.confidence);
        ...
        case EuropeanDriverLicenseBack.DOCUMENT_TYPE:
          ...
          addField('Restrictions', doc.restrictions?.value?.text,
              doc.restrictions?.value?.confidence);
          addField('Categories A1', doc.categories.a1.validFrom?.value?.text,
              doc.categories.a1.validFrom?.value?.confidence);
          addField('Categories A2', doc.categories.a2.validFrom?.value?.text,
              doc.categories.a2.validFrom?.value?.confidence);
          addField('Categories B1', doc.categories.b1?.validFrom?.value?.text,
              doc.categories.b1?.validFrom?.value?.confidence);
          //...
          break;
        default:
          final stack = <GenericDocument>[document];
          while (stack.isNotEmpty) {
            final doc = stack.removeLast();
            for (final field in doc.fields) {
              final value = field.value?.text ?? '—';
              final conf = field.confidenceWeight;
              addField(field.type.name, value, conf);
            }
            stack.addAll(doc.children);
          }
          break;
      }
    }
    ...
    return Scaffold(
      appBar: ScanbotAppBar('Extracted Documents Data',
